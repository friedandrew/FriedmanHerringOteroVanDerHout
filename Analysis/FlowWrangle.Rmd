...
setting up for creating a function to repeat the above
(possibly not necessary)
```{r}
#reference Nick code 
```

writing the function with defined pieces from above
**NOTE** 
...
dam_site_4_intersect <- dam_complete_raw_crs[lengths(st_intersects(dam_complete_raw_crs, Site_4_catchment)) > 0,]

ggplot(Site_4_catchment) +
    geom_sf(fill = "white", color = "gray", lwd = 0.5) +
    geom_sf(data = Site_4_flow) +
    geom_sf(data = dam_site_4_intersect, aes(color = Fish.Passage.Catagory))+
    labs(color = "Stream Order") 
